...
class FakeNewsNetworkBuilder:
    def __init__(self, dataset_path='C:/Users/32322/OneDrive/Desktop/381k_firstgraph_file'):
    ...
    def generate_users(self, fake_tweets, real_tweets, num_users=50000):
        ...
        for user_id in tqdm(range(1, num_users + 1), desc="Generating users"):
    ...
    def build_user_news_graph(self, datasets, news_to_tweets):
        ...
        G = nx.Graph()
        # Track shares separately by source + truth label
            ...
            if user_row['tweet_ids']:
                ...
                for tweet_id in tweet_ids:
                    if tweet_id in tweet_to_news:
                        ...
                        source = G.nodes[news_id]['source']          # "politifact" or "gossipcop"
                        label = G.nodes[news_id]['ground_truth']     # "fake" or "real"
                        key = f"{source}_{label}"                    # e.g., "politifact_fake"
                        user_sharing_history[user_id][key] += 1
                       
        
        self.G = G
        return G, user_sharing_history

    def assign_user_share_features(self, user_sharing_history):
        """Assign share count features to user nodes"""
        print("Assigning user share features...")

        for user_id, counts in user_sharing_history.items():
            total = sum(counts.values())

            if self.G.has_node(user_id):
                self.G.nodes[user_id]['politifact_fake'] = counts['politifact_fake']
                self.G.nodes[user_id]['politifact_real'] = counts['politifact_real']
                self.G.nodes[user_id]['gossipcop_fake'] = counts['gossipcop_fake']
                self.G.nodes[user_id]['gossipcop_real'] = counts['gossipcop_real']
                self.G.nodes[user_id]['total_shares'] = total
    ...
    def get_user_feature_matrix(self):
        """Return user feature matrix (numpy) and an index mapping"""
        user_nodes = [n for n in self.G.nodes if self.G.nodes[n]['node_type'] == 'user']
        user_nodes.sort()  # ensure stable ordering

        features = []
        for u in user_nodes:
            node = self.G.nodes[u]
            features.append([
                node.get('politifact_fake', 0),
                node.get('politifact_real', 0),
                node.get('gossipcop_fake', 0),
                node.get('gossipcop_real', 0),
                node.get('total_shares', 0)
            ])

        return np.array(features, dtype=float), user_nodes

    """Return user feature matrix (numpy) and an index mapping"""

def main():
    # Initialize builder
    builder = FakeNewsNetworkBuilder(
    dataset_path=r"C:\Users\32322\OneDrive\Desktop\381k_firstgraph_file"
    )
    ...
    builder.assign_user_share_features(user_sharing_history)
